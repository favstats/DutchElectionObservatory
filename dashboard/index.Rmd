---
title: '`r readRDS("data/trans_dutch.rds")$dashboard_title`'
output:
  flexdashboard::flex_dashboard:
    orientation: row
    vertical_layout: scroll
    smooth_scroll: true
    theme: bootstrap
    logo: https://politieke-advertenties.nl/wp-content/uploads/2020/11/logo-e1604404707339.png
    favicon: https://politieke-advertenties.nl/wp-content/uploads/2020/11/logo-e1604404707339.png
---

```{r, context = "server", eval=F}
library(shinyWidgets)

observe({
  sendSweetAlert(session, 
                 title = trans$welcome_title,
                 text = tags$span(
                   trans$welcome_text_one,
                   tags$a(trans$welcome_text_two,     href="https://politieke-advertenties.nl/waar-komen-onze-data-vandaan/", target="_blank"), 
                   trans$welcome_text_three,
                   tags$br(),
                   tags$br(),
                   trans$welcome_text_four), html = T)
 }) 

```

**About**
=========================================

## Report

```{r}
library(shiny)
# renderUI({
  shiny::HTML('<iframe style="height:150%; width:100%"; src="about.html"></iframe>')
# })


```


**Number of Ads**
=========================================

## Report

```{r}
library(shiny)
# renderUI({
  shiny::HTML('<iframe style="height:150%; width:100%"; src="number_of_ads.html"></iframe>')
# })


```



**Money Spent**
=========================================

## Report

```{r}
library(shiny)
# renderUI({

shiny::HTML('<iframe style="height:150%; width:100%"; src="money.html"></iframe>')

# })

```




**Impressions**
=========================================

## Report

```{r}
library(shiny)
# renderUI({

shiny::HTML('<iframe style="height:150%; width:100%"; src="impressions.html"></iframe>')

# })

```



**Targeting**
=========================================

## Report

```{r}
library(shiny)
# renderUI({

shiny::HTML('<iframe style="height:150%; width:100%"; src="targeting.html"></iframe>')

# })

```

