...
#   )
  ...
  sendSweetAlert(session,     
                 title = "Welcome to the Dutch Election Observatory Dashboard",
...
# bsTooltip("radio_A", title="Tooltip for A")
  ...
  # more_than_5_fb <-  fb_aggr$total %>% 
  # filter()
  # drop_na(advertiser_id) %>% 
  # arrange(desc(advertiser_name)) %>% 
  # slice(20:nrow(.)) %>% 
  # filter(n >= 5) %>% 
  # arrange(advertiser_name) %>% 
  # filter(!str_detect(advertiser_name, "𝒫")) %>% 
  # filter(!str_detect(advertiser_name, "yolanda_dxb")) %>% 
...
var_objs_fb <- fb_aggr$total %>%
  drop_na(advertiser_id) %>% 
  # filter(advertiser_name %in% more_than_5_fb) %>% 
  mutate(advertiser_name = str_remove_all(advertiser_name, "[^[:alnum:] ]")) %>% 
  mutate(java_script = paste0("'", advertiser_name, "': ", "'https://www.facebook.com/", advertiser_id, "'")) %>% 
  pull(java_script) %>%
  paste0(collapse = ",")

js_scrip_fb <- glue::glue("function() { 
            var obj = {
                     <<var_objs_fb>>
                   }; 
            ...
            return '<a target=\"_top\" href=\"'+ getProperty(this.value) + '\">' + this.value + '</a>';}", .open = "<<", .close = ">>")%>% as.character()
...
selectizeInput("advertisers__fb", "Select Advertisers", 
  ...
  # updateSelectizeInput(session = session,inputId ="advertisers__fb",choices=sort(unique(mtcars$cyl)),selected=sort(unique(mtcars$cyl))[1], server = TRUE)
...
# observeEvent( onclick("advertisers__fb", {
...
# debugonce(hc_plotter)
  ...
  # get_url <- function(variables) {
  #   
  # }
  # 
  # library(rvest)
  # 
  # doc.retd    <- read_xml("data/retract.xml")
  # 
  # 
  # dat <- doc.retd %>% 
...
var_objs_ggl <- ggl_aggr$total %>% 
  mutate(java_script = paste0("'", advertiser_name, "': ", "'https://transparencyreport.google.com/political-ads/advertiser/", advertiser_id, "'")) %>% 
  pull(java_script) %>% 
  paste0(collapse = ",")

js_scrip_ggl <- glue::glue("function() { 
            var obj = {
                     <<var_objs_ggl>>
                   }; 
                  var getProperty = function (propertyName) {
...
selectizeInput("advertisers__ggl", "Select Advertisers", 
