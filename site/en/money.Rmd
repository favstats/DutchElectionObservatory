...
For Facebook platforms, spending data from the API does not provide aggregate statistics per advertiser. Instead, it only provides very broad boundaries of money spent. For this reason, the data displayed here is from the [Facebook Ad Library Report
](https://www.facebook.com/ads/library/report) which does provide money spent by advertiser and is retrieved daily.
...
For Facebook platforms, spending data from the API does not provide aggregate statistics per advertiser. Instead, it only provides very broad boundaries of money spent. For this reason, the data displayed here is from the [Facebook Ad Library Report
](https://www.facebook.com/ads/library/report) which does provide money spent by advertiser and is retrieved daily. In addition, the data also shows spending by region.
...
```
For Facebook platforms, spending data from the API does not provide aggregate statistics per advertiser. Instead, it only provides very broad boundaries of money spent. For this reason, the data displayed here is from the [Facebook Ad Library Report
](https://www.facebook.com/ads/library/report) which does provide money spent by advertiser and is retrieved daily. However, one caveat is that if a perty spent below 100 Euros in a day it will not show the exact amount. For this reason we show confidence intervals around the amount.
